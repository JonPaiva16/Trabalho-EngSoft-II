"""
Camada de dados da API de livros.
"""
...
class RepositorioEmMemoria(RepositorioLivros):
    """Repositorio em memoria usado pela API e pelos testes."""
    ...
    def get_by_isbn(self, isbn: str) -> Livro | None:
        return next(
            (livro for livro in self._livros.values() if livro.isbn == isbn),
            None,
        )
    ...
    def adicionar(self, dados: LivroCriar) -> Livro:
        novo = Livro(
            id=self._proximo_id,
            titulo=dados.titulo or "",
            autor=dados.autor or "",
            ano=dados.ano,
            isbn=dados.isbn,
        )
